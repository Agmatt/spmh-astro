...
const VALIDATION = {
  ...
  phone: (phone) => {
    // Accept various phone formats: +254..., 254..., 0..., (###)###-####, etc.
...
export default function TenderApplicationForm({ tenderCode }) {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      const { error: insertError } = await supabase.from('tender_applications').insert({
        tender_id: tender.id,
      ...
      setGeneralError(
        err.message || 'Something went wrong submitting your application. Please try again or contact procurement@spmh.co.ke'
      );
    } finally {
      setSubmitting(false);
    }
  ...
  if (!tender) {
    return (
      <div class="max-w-lg mx-auto text-center py-16 border border-dashed border-slate-300 rounded-lg bg-slate-50">
        <p class="text-lg font-semibold text-slate-700 mb-2">Tender Not Found</p>
        <p class="text-sm text-slate-500 mb-6">
          This tender is no longer open for applications, or the link is invalid.
        </p>
        <a href="/tenders" class="text-amber-600 font-bold hover:underline">
          ← Browse all tenders
        </a>
      </div>
    );
  ...
  if (submitted) {
    return (
      <div class="max-w-lg mx-auto py-16">
        <div class="bg-gradient-to-br from-teal-50 to-emerald-50 border border-teal-200 rounded-lg p-4 md:p-8 text-center">
          <div class="text-5xl mb-4">✓</div>
          <h2 class="text-xl font-serif font-bold text-slate-900 mb-2">Application Submitted</h2>
          <p class="text-slate-600 mb-6">
            Your submission for <strong>{tender.tender_code}</strong> has been received and recorded securely.
          </p>
          <div class="bg-white rounded p-4 mb-6 text-left text-sm text-slate-600 border border-teal-100">
            <p class="mb-2">
              <strong>What happens next:</strong>
            </p>
            <ul class="space-y-1 ml-4 list-disc">
              <li>Our procurement team will review your submission</li>
              <li>All applicants will be notified of outcomes by {tender.closing_date}</li>
            </ul>
          </div>
          ...
          <a
            ...
            class="inline-block bg-teal-600 text-white font-bold px-6 py-2 rounded-lg hover:bg-teal-700 transition-colors"
          >
    ...
    <div class="max-w-lg mx-auto">
      <div class="bg-white rounded-lg border border-slate-200 shadow-sm overflow-hidden">
        ...
        <div class="bg-gradient-to-r from-slate-50 to-blue-50 p-6 border-b border-slate-200">
          <span class="text-xs font-bold text-accent">{tender.tender_code}</span>
          <h2 class="text-xl font-serif font-bold text-slate-900 mt-1">{tender.title}</h2>
          <p class="text-xs text-slate-500 mt-2">Submit your application below</p>
        </div>
        ...
        <form onSubmit={handleSubmit} class="p-6 space-y-5">
          ...
          <div>
            ...
            <input
              ...
              class={`w-full border-2 rounded px-4 py-2 text-sm transition-colors ${
                errors.company_name ? 'border-accent bg-red-50' : 'border-slate-200 hover:border-slate-300 focus:border-accent'
              } focus:outline-none`}
            />
            {errors.company_name && <p class="text-xs text-red-600 mt-1">{errors.company_name}</p>}
          </div>
          ...
          <div>
            ...
            <input
              ...
              class={`w-full border-2 rounded px-4 py-2 text-sm transition-colors ${
                errors.contact_name ? 'border-red-400 bg-red-50' : 'border-slate-200 hover:border-slate-300 focus:border-amber-400'
              } focus:outline-none`}
            />
            {errors.contact_name && <p class="text-xs text-red-600 mt-1">{errors.contact_name}</p>}
          </div>
            ...
            <div>
              ...
              <input
                ...
                class={`w-full border-2 rounded px-3 py-2 text-sm transition-colors ${
                  errors.email ? 'border-red-400 bg-red-50' : 'border-slate-200 hover:border-slate-300 focus:border-amber-400'
                } focus:outline-none`}
              />
              {errors.email && <p class="text-xs text-red-600 mt-1">{errors.email}</p>}
            </div>
            <div>
              <label class="block text-xs font-bold uppercase tracking-widest text-slate-600 mb-2">
                Phone
              ...
              />
              {errors.phone && <p class="text-xs text-red-600 mt-1">{errors.phone}</p>}
            </div>
          </div>
          ...
          <div>
            ...
            <label class="block border-2 border-dashed border-slate-300 rounded px-4 py-6 text-center cursor-pointer hover:border-amber-400 hover:bg-amber-50 transition-colors">
              <input
                type="file"
              ...
              {file ? (
                ...
                <div>
                  <p class="text-sm font-semibold text-slate-700 mb-1">📄 Click to upload PDF</p>
                  <p class="text-xs text-slate-500">Max 5MB, PDF format only</p>
                </div>
              )}
            </label>
            {fileError && <p class="text-xs text-red-600 mt-2">{fileError}</p>}
          </div>

          {/* Privacy Notice */}
          <div class="bg-slate-50 border border-slate-200 rounded p-4">
            <p class="text-xs text-slate-600">
              <strong>Privacy & Security:</strong> Your information is encrypted and handled per Kenya's PDPA. We only use it to evaluate your application.
            </p>
          </div>
          ...
          <button
            type="submit"
            disabled={submitting}
            class="w-full bg-gradient-to-r from-amber-400 to-amber-500 text-slate-900 font-bold py-3 rounded-lg hover:shadow-lg transition-all disabled:opacity-50 disabled:cursor-not-allowed"
          >
