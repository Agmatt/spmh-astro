...
const statCardDefs = [
    { label: 'Total Volunteers', key: null, color: C.dark },
    { label: 'Active', key: 'active', color: '#065f46' },
    { label: 'Pending', key: 'pending', color: '#92400e' },
    { label: 'Inactive', key: 'inactive', color: '#7f1d1d' },
];
...
const VolunteersAdminDashboard = () => {
    ...
    const checkAuth = async () => {
        try {
            ...
            if (session?.user) { setUser(session.user); setAuthState('dashboard'); fetchVolunteers(); }
    ...
    const handleLogin = async (e) => {
        ...
        try {
            ...
            if (data.user) { setUser(data.user); setAuthState('dashboard'); setEmail(''); setPassword(''); fetchVolunteers(); }
        } catch (err) { setLoginError('Login failed. Please try again.'); }
    };

    const handleLogout = async () => {
        await supabase.auth.signOut();
        setUser(null); setAuthState('login'); setVolunteers([]); setFilteredVolunteers([]);
    };
    ...
    const updateVolunteerStatus = async (id, newStatus) => {
        try {
            ...
            if (error) throw error;
            fetchVolunteers();
        } catch (err) { console.error('Error updating volunteer:', err); }
    };
        ...
        return (
            <div className="min-h-screen flex items-center justify-center p-4 relative overflow-hidden"
                ...
                <div className="relative z-10 bg-white rounded-2xl shadow-2xl w-full max-w-md p-8">
                    ...
                    <p className="text-sm mb-6" style={{ color: C.muted }}>Access the volunteers dashboard</p>
            ...
            <header className="sticky top-0 z-40 border-b bg-white"
                ...
                <div className="max-w-7xl mx-auto px-4 py-3 flex flex-wrap items-center justify-between gap-3">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h1 className="text-base sm:text-lg font-bold leading-tight" style={{ color: C.dark }}>Volunteers</h1>
            ...
            <main className="max-w-7xl mx-auto px-4 py-6 space-y-6">
                    ...
                    {statCardDefs.map((card, i) => (
                        <div key={card.label} className="rounded-xl p-4 sm:p-5"
                            ...
                            <p className="text-3xl font-bold text-white">
                                {card.key === null
                                    ? volunteers.length
                                    : volunteers.filter(v => v.status === card.key).length}
                            </p>
                        </div>
                    ))}
                </div>
                ...
                <div className="bg-white rounded-xl border p-4 sm:p-6" style={{ borderColor: C.border }}>
                    ...
                    <div>
                        ...
                        <select value={statusFilter}
                            onChange={e => { setStatusFilter(e.target.value); applyFilters(volunteers); }}
                ...
                <div className="bg-white rounded-xl border overflow-hidden" style={{ borderColor: C.border }}>
                    <div className="hidden md:block overflow-x-auto">
                        <table className="w-full">
                            <thead style={{ background: C.warm, borderBottom: `1px solid ${C.border}` }}>
                                <tr>
                                    {['Name', 'Email', 'Phone', 'Location', 'Interests', 'Availability', 'Status', 'Actions'].map(h => (
                            ...
                            <tbody>
                                {filteredVolunteers.length === 0 ? (
                                    ...
                                    <tr key={vol.id}
                                        ...
                                        <td className="px-5 py-3.5 text-sm font-semibold" style={{ color: C.dark }}>{vol.first_name} {vol.last_name}</td>
                                        <td className="px-5 py-3.5 text-sm" style={{ color: C.muted }}>{vol.email || '-'}</td>
                                        <td className="px-5 py-3.5 text-sm" style={{ color: C.muted }}>{vol.phone}</td>
                                        <td className="px-5 py-3.5 text-sm" style={{ color: C.muted }}>{vol.location || '-'}</td>
                                        <td className="px-5 py-3.5 text-xs" style={{ color: C.muted }}>{vol.interests}</td>
                                        <td className="px-5 py-3.5 text-xs" style={{ color: C.muted }}>{vol.availability}</td>
                                        <td className="px-5 py-3.5">
                                            <select value={vol.status || 'pending'}
                                                ...
                                                style={statusStyle[vol.status] || {}}>
                                                {statuses.map(s => <option key={s} value={s}>{s.charAt(0).toUpperCase() + s.slice(1)}</option>)}
                                            </select>
                                        </td>
                                        <td className="px-5 py-3.5">
                                            <button onClick={() => deleteVolunteer(vol.id)}
                    ...
                    <div className="md:hidden divide-y" style={{ borderColor: '#f3f4f6' }}>
                        {filteredVolunteers.length === 0 ? (
                            <p className="px-4 py-10 text-center text-sm" style={{ color: C.mutedLt }}>No volunteers found</p>
                        ) : filteredVolunteers.map(vol => (
                            <div key={vol.id} className="p-4 space-y-2.5">
                                <div className="flex items-start justify-between gap-2">
                                    <div>
                                        <p className="text-sm font-bold" style={{ color: C.dark }}>{vol.first_name} {vol.last_name}</p>
                                        <p className="text-xs" style={{ color: C.muted }}>{vol.email || '-'}</p>
                                        <p className="text-xs" style={{ color: C.mutedLt }}>{vol.phone}</p>
                                    </div>
                                    <select value={vol.status || 'pending'}
                                        onChange={e => updateVolunteerStatus(vol.id, e.target.value)}
