  ...
  return (
    <div className='flex-1 flex flex-col items-center justify-center p-10 text-center'>
      ...
      <p className='text-gray-400 text-xs mt-1.5 max-w-xs'>
        Choose a role from the list on the left to see the full job description
        and requirements.
      </p>
    </div>
  );
...
function BulletItem({ children, check = false }) {
  return (
    <li className='flex items-start gap-2.5 text-sm text-gray-600 leading-relaxed'>
      {check ? (
        <svg
          className='w-4 h-4 text-[#7B1A2E] mt-0.5 shrink-0'
          fill='none'
          stroke='currentColor'
          viewBox='0 0 24 24'>
          <path
            ...
            <div className='px-6 py-6 space-y-8'>
              ...
              <section className='bg-gray-50 rounded-xl p-4 border border-gray-200'>
                <p className='text-xs text-gray-500 leading-relaxed'>
                  <strong className='text-gray-700'>How to apply:</strong>{' '}
                  Complete the application form by clicking the button below.
                  Attach your CV (PDF or Word, max 5MB) and an optional cover
                  letter. Only shortlisted candidates will be contacted. St.
                  Paul's Mission Hospital is an equal opportunity employer.
                </p>
              </section>
            </div>
          </div>
              ...
              <div className='flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3'>
                <div>
                  ...
                  <p className='text-xs text-gray-400 mt-0.5'>
                    Application closes {job.deadline}
                  </p>
                </div>
                <button
                  onClick={onApply}
